...
async function initDB() {
  ...
  try {
    ...
    // IMPORTANT: some databases already have Aquabike and Aquagym slots at the same date/hour.
    // We first merge bookings onto one slot, remove duplicate slots, then rename the kept slot.
    await client.query(`
      WITH grouped AS (
        SELECT date_slot, hour_start, MIN(id) AS keep_id, ARRAY_AGG(id) AS all_ids
        FROM slots
        WHERE activity IN ('Aquabike','Aquagym','Aquabike/Aquagym')
        GROUP BY date_slot, hour_start
        HAVING COUNT(*) > 1
      ), moved AS (
        UPDATE bookings b
        SET slot_id = g.keep_id
        FROM grouped g
        WHERE b.slot_id = ANY(g.all_ids)
          AND b.slot_id <> g.keep_id
        RETURNING b.id
      ), dedup_bookings AS (
        DELETE FROM bookings b
        USING bookings b2
        WHERE b.slot_id = b2.slot_id
          AND b.user_id = b2.user_id
          AND b.status = 1
          AND b2.status = 1
          AND b.id > b2.id
        RETURNING b.id
